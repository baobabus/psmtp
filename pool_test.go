...
func TestGetMailer(t *testing.T) {
	...
	sbj, cch = tc_pool1(); sbj.MaxConns = 0; sbj.MaxIdleConns = 0
	m, err = sbj.GetMailer(nil)
	if err != nil { t.Error(err); return; }
	if m == nil { t.Errorf("Expected non-nil value"); return; }
	s = <- cch; c = make(chan int); s <- c; close(s)
	err = m.Close(); c <- 1
	if err != nil { t.Error(err); return; }
	sbj.Close()
	close(cch)
	...
	sbj, cch = tc_pool1(); sbj.MaxConns = 1; sbj.MaxIdleConns = 0
	m, err = sbj.GetMailer(nil)
	if err != nil { t.Error(err); return; }
	if m == nil { t.Errorf("Expected non-nil value"); return; }
	s = <- cch; c = make(chan int); s <- c; close(s)
	...
	sbj, cch = tc_pool1(); sbj.MaxConns = 1; sbj.MaxIdleConns = 1
	m, err = sbj.GetMailer(nil)
	if err != nil { t.Error(err); return; }
	if m == nil { t.Errorf("Expected non-nil value"); return; }
	s = <- cch; c = make(chan int); s <- c; close(s)
	...
	sbj, cch = tc_pool1(); sbj.MaxConns = 2; sbj.MaxIdleConns = 0
	m1, err = sbj.GetMailer(nil)
	if err != nil { t.Error(err); return; }
	if m1 == nil { t.Errorf("Expected non-nil value"); return; }
	s = <- cch; c1 = make(chan int); s <- c1; close(s)
	err = m1.Close(); c1 <- 1
	if err != nil { t.Error(err); return; }
	m2, err = sbj.GetMailer(nil)
	s = <- cch; c2 = make(chan int); s <- c2; close(s)
	if err != nil { t.Error(err); return; }
	if m2 == nil { t.Errorf("Expected non-nil value"); return; }
	err = m2.Close(); c2 <- 1
	if err != nil { t.Error(err); return; }
	sbj.Close()
	close(cch)
	...
	if err != nil { t.Error(err); return; }
	if m1 == nil { t.Errorf("Expected non-nil value"); return; }
	s = <- cch; c1 = make(chan int, 1); s <- c1; close(s)
	m2, err = sbj.GetMailer(nil)
	s = <- cch; c2 = make(chan int, 1); s <- c2; close(s)
	if err != nil { t.Error(err); return; }
	if m2 == nil { t.Errorf("Expected non-nil value"); return; }
	// Expecting "connection refused" delayed due to async dialing
	err = m2.Close(); c2 <- 1
	if err == nil { t.Errorf("Expected error, but was successful"); return; }
	err = m1.Close(); c1 <- 1
	if err != nil { t.Error(err); return; }
	m2, err = sbj.GetMailer(nil)
	s = <- cch; c2 = make(chan int, 1); s <- c2; close(s)
...
func (this net_Conn) Read(b []byte) (int, error) { time.Sleep(this.latency); if this.cmd != nil { <- this.cmd; }; return 0, nil; }
func (this net_Conn) Write(b []byte) (int, error) { time.Sleep(this.latency); if this.cmd != nil { <- this.cmd; }; return 0, nil; }
func (this net_Conn) Close() error {
	time.Sleep(this.latency)
	if this.cmd != nil { <- this.cmd; } 
	if this.pool != nil {
		select {
		case this.pool <- 1:
		default:
...
func (this *net_smtp_Client) Auth(a smtp.Auth) error { time.Sleep(this.conn.latency); return this.errAuth; }
func (this *net_smtp_Client) Close() error {
	this.conn.Close()
	return this.errClose
}
func (this *net_smtp_Client) Data() (io.WriteCloser, error) { time.Sleep(this.conn.latency); return nil, this.errData; }
func (this *net_smtp_Client) Extension(ext string) (bool, string) { return true, ext; }
func (this *net_smtp_Client) Hello(localName string) error { time.Sleep(this.conn.latency); return this.errHello; }
func (this *net_smtp_Client) Mail(from string) error { time.Sleep(this.conn.latency); return this.errMail; }
...
func (this *net_smtp_Client) StartTLS(config *tls.Config) error { time.Sleep(this.conn.latency); return this.errStartTLS; }
func (this *net_smtp_Client) Verify(addr string) error { time.Sleep(this.conn.latency); return this.errVerify; }
...
func net_smtp_NewClient_fake(c *net_smtp_NewClient_case) func (net.Conn, string) (smtpClient, error) {
	return func(conn net.Conn, server string) (smtpClient, error) {
		if conn != nil { time.Sleep(conn.(*net_Conn).latency); }
